...
public interface TransitDAO extends JpaRepository<Transit, Integer>{
    ...
    @Query(value ="select ROW_NUMBER() OVER (\n" +
            ...
            "    FROM db_traffic_lviv.transit join crossing_point on crossing_point.id = transit.in_id \n" +
            "    where start_time >= ?1  \n" +
    ...
    @Query(value ="SELECT in_id as id, crossing_point.road_name, sum(amount) as amount, date(start_time) as start_time, date(end_time) as end_time FROM db_traffic_lviv.transit join crossing_point on crossing_point.id = transit.in_id  where date(start_time) = ?1  group by in_id order by amount desc", nativeQuery = true)
