...
public interface AmountDAO extends JpaRepository<Amount, Integer> {
    ...
    @Query(value = "SELECT id, convert(start_time, DATE) as start_time, convert(end_time, DATE) as end_time, amount FROM vehicles_amount where start_time >=?1 and start_time=end_time", nativeQuery = true)
