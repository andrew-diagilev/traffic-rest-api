...
public interface AmountDAO extends JpaRepository<Amount, Integer> {
    @Query(value = "select * from vehicles_amount where start_time >= ?1 and end_time <= ?2 and start_time <> end_time", nativeQuery = true)
    ...
    @Query(value = "SELECT amount FROM vehicles_amount where start_time=end_time ORDER BY ID DESC LIMIT 1", nativeQuery = true)
