...
public interface SpeedDAO extends JpaRepository<Speed, Integer>{
    ...
    @Query(value ="SELECT id, truncate(avg(vehicles_speed), 1) as vehicles_speed,  date(start_time) as start_time, date(end_time) as end_time FROM db_traffic.vehicles_speed where date(start_time) >= ?1 group by date(start_time) order by start_time", nativeQuery = true)
